...
export default async function AlunosPage() {
  ...
  if (alunoIds.length > 0) {
    const { data } = await supabase.from("profiles").select("*").in("id", alunoIds)
    profiles = data ?? []
  }
    ...
    <div className="p-8">
      ...
      {alunosComStatus.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">Convide alunos pelo botão acima para começar.</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {alunosComStatus.map((aluno) => (
                <tr key={aluno.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <a href={`/dashboard/alunos/${aluno.id}`} className="text-blue-600 hover:underline text-sm">
                      Ver perfil →
                    </a>
                  </td>
                </tr>
