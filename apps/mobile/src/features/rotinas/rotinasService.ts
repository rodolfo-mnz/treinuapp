...
export async function buscarRotinaDia(rotinaDiaId: string) {
  const { data, error } = await supabase
    .from("rotina_dias")
    .select(`
      *,
      rotina:rotinas(id, nome, objetivo),
      rotina_exercicios (
        *,
        exercicio:exercicios (*)
      )
    ...
    .single();
  if (error) throw error;
  const dia = data as typeof data & {
    rotina_exercicios: (typeof data.rotina_exercicios[0])[];
  };
  dia.rotina_exercicios.sort((a, b) => a.ordem - b.ordem);
  return dia;
}
